...
public class InformationPanel extends JPanel{
	...
	public void updatelabelPoints(){
		labelPoints.setText("Puntos: "+play.getPlayer().getPoints()+" - Vidas: "+play.getPlayer().getLifes());
	}
	public void deleteImageLabel(){
		labelEnemies[actualImage].setIcon(null);
		if(actualImage>0){
			actualImage--;
		}
	}
			...
			if((botones[0].getName()==s)||(botones[0].getText()==s)){
				...
				if((botones[1].getName()==s)||(botones[1].getText()==s)){
					...
					if(!vacio){
						...
						play.getPlayer().setPoints(play.getMap().getEnemies()[x].getPoints());
					...
					for(int i=0;(i<obs[0].length)&&(!found);i++){
						for(int j=0;(j<obs.length)&&(!found);j++){
							if(obs[i][j]!=null){
								cantObs++;
								z=i;
								y=j;
					...
					if(cantObs>1){
						if(play.getMap().getMap()[z][y]!=null){
							play.getMap().getMap()[z][y].collide(play.getPlayer());
						}
					}
					else{
